...
/*
 * Configure pads for eMMC memory
 */
#if defined(CONFIG_SOMLABS_VISIONSOM_6ULL_EMMC)
...
#if defined(CONFIG_SOMLABS_VISIONSOM_6ULL_EMMC)
		ret = 1;
#elif defined (CONFIG_SOMLABS_VISIONSOM_6ULL_SD)
...
static iomux_v3_cfg_t const wdog_b_pad = {
	MX6_PAD_ENET2_RX_ER__WDOG1_WDOG_ANY | MUX_PAD_CTRL(WDOG_PAD_CTRL)
};
...
int board_late_init(void)
{
	...
	switch (get_boot_device()) {
		...
		default:
			if(is_im6ull) {
				setenv("fdt_file", "somlabs-visionsom-6ull.dtb");
			} else {
				setenv("fdt_file", "somlabs-visionsom-6ul.dtb");
			}
			break;
		case MMC1_BOOT:
		case MMC2_BOOT:
			if(is_im6ull) {
				setenv("fdt_file", "somlabs-visionsom-6ull-emmc.dtb");
			} else {
				setenv("fdt_file", "somlabs-visionsom-6ul-emmc.dtb");
			}
			break;
		case NAND_BOOT:
			if(is_im6ull) {
				setenv("fdt_file", "somlabs-visionsom-6ull-nand.dtb");
			} else {
				setenv("fdt_file", "somlabs-visionsom-6ul-nand.dtb");
			}
			break;
	}
